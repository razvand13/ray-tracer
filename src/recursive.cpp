...
Ray generateReflectionRay(Ray ray, HitInfo hitInfo)
{
    ...
    reflected.origin = reflected.origin + 20.f * glm::epsilon<float>() * reflected.direction; // offset to reduce noise
...
Ray generatePassthroughRay(Ray ray, HitInfo hitInfo)
{
    ...
    passthrough.origin = passthrough.origin + 20.f * glm::epsilon<float>() * passthrough.direction; // offset to reduce noise
